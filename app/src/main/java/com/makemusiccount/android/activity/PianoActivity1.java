...
public class PianoActivity1 extends AppCompatActivity implements OnPianoListener,
    ...
    protected void onResume() {
        ...
        pianoView=new PianoView1(context);
        ...
        pianoView.autoplayclickview=true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
            context = this;
        setTheme(Util.getTheme(context));             setContentView(      R.layout.activity_piano1);
        ...
        PianoActivity1.setPianoClickListener(Position -> {
    ...
    public void onPianoClick(Piano.PianoKeyType type, Piano.PianoVoice voice, int group, int positionOfGroup) {
        ...
        if (onPianoClickListener != null) {
            if (!autoPlay) {
                if(position==Integer.parseInt(recordPianoKeyLists.get(iii).getPosition()))
                {
                    slideDown(view);
                    onPianoClickListener.onClick(position);
                    iii++;
                    if(recordPianoKeyLists.size()>iii)
                    {
    ...
    private void startAutoPlay() {
        ...
        view =getviewiii(iii);
        nextview= getviewiii(iii);
        RecordPianoKeyList key = recordPianoKeyLists.get(iii);
        delay=delay+Integer.parseInt(key.getWaiting_time());
