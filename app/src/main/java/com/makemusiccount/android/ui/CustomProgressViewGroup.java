...
public class CustomProgressViewGroup extends ViewGroup {
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    float amplitude = mHeight / 10;

    //handling width
    int waveCount = total_wave_count; //subtracting the background view child
    ...
    for (int i = 0; i < count; i++) {
      ...
      //handling case of linear layout child views
        ...
        View imageView = (TextView) linearChild.getChildAt(1);

        final int imageTop = imageView.getPaddingTop();
        final int imageBottom = imageView.getMeasuredHeight() ;
        final int imageHeight = imageBottom - imageTop;

        final int textTop = textView.getPaddingTop();
        final int textBottom = textView.getMeasuredHeight() ;
